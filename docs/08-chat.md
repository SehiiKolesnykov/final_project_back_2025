...
Видалити весь чат-тред з усіма повідомленнями (потрібна автентифікація).

#### Path Parameters

| Параметр | Тип | Опис |
|----------|-----|------|
| threadId | Long | ID треда, який потрібно видалити |

#### Response

**Status**: `200 OK` - Тред успішно видалено

**Status**: `404 Not Found` - Тред не знайдено

**Status**: `403 Forbidden` - Користувач не є учасником треду

#### Примітки

- Видаляє весь тред разом з усіма повідомленнями та статусами прочитання
- Доступно тільки для учасників треду
- Операція незворотна - дані видаляються безповоротно
- Після видалення треду він зникне зі списку тредів обох учасників

---

### DELETE /api/chat/message/{messageId}

Видалити окреме повідомлення з чату (потрібна автентифікація).

#### Path Parameters

| Параметр | Тип | Опис |
|----------|-----|------|
| messageId | Long | ID повідомлення, яке потрібно видалити |

#### Response

**Status**: `200 OK` - Повідомлення успішно видалено

**Status**: `404 Not Found` - Повідомлення не знайдено

**Status**: `403 Forbidden` - Користувач не є автором повідомлення

#### Примітки

- Видаляє тільки одне конкретне повідомлення
- Доступно тільки для автора повідомлення
- Операція незворотна - повідомлення видаляється безповоротно
- Після видалення повідомлення воно зникне з історії чату для обох учасників
...
1. Викликати `DELETE /api/chat/thread/{threadId}` для видалення всього чату
2. Переконатися, що користувач є учасником треду
3. Після видалення тред зникне зі списку розмов

### Видалення повідомлення

1. Викликати `DELETE /api/chat/message/{messageId}` для видалення конкретного повідомлення
2. Переконатися, що користувач є автором повідомлення
3. Після видалення повідомлення зникне з історії чату
...
- Видалення треду або повідомлення є незворотною операцією
- Тред можуть видаляти тільки його учасники
- Повідомлення можуть видаляти тільки їх автори
